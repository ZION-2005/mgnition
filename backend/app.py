...
def recommend_variants(answers, top_k=6):
    ...
    for v in variants:
        base_rule = rule_score(v, answers)
        ...
        ranked.append({**v, "rule_score": round(base_rule, 4), "ml_score": ml_score, "score": round(final, 6)})
...
def public_promotions():
    rows = get_db().execute(
        """
        SELECT id, title, description, price_text, badge_text, image_url, start_date, end_date
        FROM promotions
        WHERE active = 1
        ORDER BY id DESC
        """
...
def admin_add_promotion():
    ...
    db.execute(
        """
        INSERT INTO promotions (title, description, price_text, badge_text, image_url, start_date, end_date, active, created_by, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
        """,
        (
            title,
